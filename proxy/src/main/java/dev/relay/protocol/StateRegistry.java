...
public final class StateRegistry {
    ...
    static {
        ...
        PLAY.serverbound.register("chat_command", ChatCommandPacket.class, ChatCommandPacket::new,
                map(0x04, V1_20_2), map(0x04, V1_20_5), map(0x04, V1_21_2), map(0x05, V1_21_4));
        PLAY.serverbound.register("configuration_acknowledged", ConfigurationAcknowledgedPacket.class,
                ...
                map(0x0B, V1_20_2), map(0x0C, V1_20_5), map(0x0D, V1_21_2), map(0x0E, V1_21_4));

        // Plugin messages in play state, for the client API (docs/client-api.md).
        //
        // Serverbound must be decoded, so its id has to be right: it sits immediately
        // after Close Container, four past Acknowledge Configuration, and it tracks that
        // packet's shifts exactly (1.20.5 inserted Cookie Response near the top of the
        // list, and 1.21.2/1.21.4 each added one more before this point).
        PLAY.serverbound.register("plugin_message", PluginMessagePacket.class, PluginMessagePacket::new,
                map(0x0F, V1_20_2), map(0x10, V1_20_5), map(0x11, V1_21_2), map(0x12, V1_21_4));
        ...
        PLAY.clientbound.registerWriteOnly("plugin_message", PluginMessagePacket.class,
                map(0x18, V1_20_2), map(0x19, V1_20_5), map(0x18, V1_21_2), map(0x19, V1_21_5));
        ...
        // ("OUT: [play:103] ClientboundSystemChatPacket"). The later values keep the
        // +2 offset from start_configuration that holds at 1.20.2; see the warning in
        // docs/protocol-ids.md -- only the 1.20.2 entry is verified.
        PLAY.clientbound.registerWriteOnly("system_chat", SystemChatPacket.class,
                map(0x67, V1_20_2), map(0x69, V1_20_3), map(0x6B, V1_20_5), map(0x72, V1_21_2),
                map(0x73, V1_21_5));
